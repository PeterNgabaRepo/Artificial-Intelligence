...
def readIn():
	maze = []
	inputType = input("1 - Filename \n2 - Maze \n")

	EOF = ""

#	filename = input("Enter filename or maze: ")
	...
	if inputType == "1":
		...
		line = input("Enter matrix, press [Enter] once after entering matrix: ")
		while(line != EOF):
			line2 = []
			for i in range(len(line)):
				if line[i] != "\n" and line[i] != " ":
...
def DFS(graph, v):
	...
	for j in range(len(graph.adjList[index])):
		...
		if graph.vertices[index2].parent == -1:
			...
			x = graph.adjList[index][j].vert2.x
			y = graph.adjList[index][j].vert2.y
...
def A(graph, s, t):
	...
	while len(queue) != 0:
		...
		x = graph.vertices[index].x
		y = graph.vertices[index].y
...
def findRoot(maze):
	for i in range(len(maze)):
		for j in range(len(maze[i])):
			if (maze[i][j] == "2" or maze[i][j] == "0")and i == 0:
				print(0)
				root = vertex(j, i)
				return root
...
def findGoal(maze):
	for i in range(len(maze)):
		for j in range(len(maze[i])):
			if (maze[i][j] == "2" or maze[i][j] == "0" )and i == len(maze) - 1:
				print(1)
				goal = vertex(j, i)
				return goal
	goal = vertex(j, i)
...
if __name__ == '__main__':
	...
	m = int(graph.getIndex(goal))
	n = int(graph.getIndex(root))

#	goal = graph.vertices[graph.getIndex(goal)]
#	root = graph.vertices[graph.getIndex(root)]

	goal = graph.vertices[m]
	root = graph.vertices[n]
